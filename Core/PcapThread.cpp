 ...
 */
void pcapLoop(unsigned char *param, const struct pcap_pkthdr *header, const uchar *pkt_data);
void pcapLoop(unsigned char *param, const struct pcap_pkthdr *header, const uchar *pkt_data)
{
 ...
 * @note pkt_data指向的数据包结构：  以太网首部[14] + IP包首部[20] + TCP包首部[20] + 正文内容[...]
 */
void PcapThread::Loop(const struct pcap_pkthdr *header, const unsigned char *pkt_data)
{

    const struct sniff_ethernet *ethernet;  // 以太网首部
    const struct sniff_ip *t_ip;            // IP字段首部
    int size_ip;                            // IP头长度
    ...
    size_ip = IP_HL(t_ip) * 4;
    if (size_ip < 20) {
        //qDebug() << QString("   * Invalid IP header length: %1 bytes").arg(size_ip);
        return;
    }

    _MessageContent t_MsgCon;
    QByteArray payload;	//有效载荷数据
    // 确定协议
    switch (t_ip->ip_p) {
    case IPPROTO_TCP:
        t_MsgCon = TCP(t_ip, size_ip, header->len, pkt_data, payload);

        if (t_MsgCon.Length > 0)
        {
            if (t_MsgCon.Length == 1 && payload.at(0) == 0x00)	//过滤长度=1并且数据为0的数据包（可能是握手成功的数据包）
                break;

            //时差
            t_MsgCon.TimeDifference = getTimeDifference(header->ts.tv_sec, header->ts.tv_usec);
            emit signal_PayloadData(t_MsgCon, payload);
        }
        break;
    case IPPROTO_UDP:
        t_MsgCon = UDP(t_ip, size_ip, pkt_data, payload);
        if (t_MsgCon.Length > 0)
        {
            if (t_MsgCon.Length == 1 && payload.at(0) == 0x00)	//过滤长度=1并且数据为0的数据包（可能是握手成功的数据包）
                break;
    ...
    //TODO: WW 待添加的时间戳信息，用于描述每一条独立的记录信息
    //compute time
    //struct tm *ltime;
    //char timestr[16];
    //time_t local_tv_sec;
